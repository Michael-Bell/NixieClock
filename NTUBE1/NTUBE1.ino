...
void loop() {
  ...
  if (resettime > 8 && resettime < 13) {
    updateColors(maxBrightness, maxBrightness, minBrightness);
    // Display the pixels on the LED strip
    delay(100);
    updateColors(minBrightness, maxBrightness, maxBrightness);
    // Display the pixels on the LED strip
    delay(100);
    updateColors(maxBrightness, minBrightness, maxBrightness);
    // Display the pixels on the LED strip
  }
  if(resettime==13)resettime=24;
  if (resettime >= 24) {
    steps=0;
